  ...
  return (
    <Box
      ...
      <form onSubmit={editAdmin}>
        ...
        >
          <TextField fullWidth id="outlined-basic" label="Username" value={username} onChange={(e) => { setUsername(e.target.value) }} placeholder="Entrer le Nom d'utilisateure " required variant="outlined" />
        </Box>
        <Box
          sx={{
            minWidth: '100%',
...
const SupAdmin = ({ row: rowProp }) => {
  ...
  const deleteAdmin = async () => {
    ...
    await axios({
      method: "delete",
      url: "http://localhost:8000/api/director/deleteSuperAdmin/" + rowProp.id,
      headers: {
        "Accept": "application/json",
        "Authorization": 'Bearer ' + accesToken
      }
...
export default function SupDirector() {
